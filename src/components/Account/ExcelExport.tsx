...
export function ExcelExport({ logContent, fileName = 'box_data' }: ExcelExportProps) {
    ...
    const handleExport = () => {
        ...
        try {
            ...
            const attrNames = ['', '星级', '等级', 'rank', '装备', 'ub', 'sk1', 'sk2', 'ex', '专武1', '专武2', '碎片', '金碎'];

            // 添加角色列到表头
            UnitIds.forEach((unitId) => {
                const unitName = unitMap.get(unitId) ?? `角色${unitId}`;
                for (let i = 0; i < 13; i++) headerRow1.push(createCell(i === 1 ? unitName : '', 'center'));
            ...
            allUserData.forEach((userData, userIndex) => {
                ...
                UnitIds.forEach((unitId) => {
                    ...
                    if (unit?.owned) {
                        [unit.rarity, unit.level, unit.rank, unit.equip, unit.ub, unit.sk1, unit.sk2, unit.ex, unit.unique_equip, unit.unique_equip2, unit.memory, unit.pure_memory].forEach((attr) =>
            ...
            worksheet['!cols'] = [
                ...
                ...Array(UnitIds.length * 13).fill({ wch: 8 }),
            ];
            worksheet['!merges'] = [];
